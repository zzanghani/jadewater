...
import { archiveBoardPostAction } from "@/app/(app)/board/actions";

export default function BoardArchiveButton({ postId }: { postId: string }) {
  ...
  function handleClick(e: React.MouseEvent) {
    ...
    if (
      !window.confirm(
        "이 글을 구글드라이브로 옮기고 여기서는 삭제합니다. 되돌릴 수 없는데 계속할까요?"
      )
    ) {
      return;
    }
    ...
    startTransition(async () => {
      const result = await archiveBoardPostAction(postId);
      if (result?.error) {
